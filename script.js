    ...
    let systemTitle = localStorage.getItem('systemTitle') || 'Smart Attendance System';
    ...
    function markAllPresent() {
      ...
      if (session === 'full-day') {
        markAllStatus('morning', 'present');
        markAllStatus('evening', 'present');
      } else {
        markAllStatus(session, 'present');
      }
    ...
    function markAllAbsent() {
      ...
      if (session === 'full-day') {
        markAllStatus('morning', 'absent');
        markAllStatus('evening', 'absent');
      } else {
        markAllStatus(session, 'absent');
      }
      
      showNotification(`Marked all as absent for ${session}`);
    }
    
    function markAllStatus(session, status) {
      const dateKey = document.getElementById('attendance-date').value;
    ...
    function exportToExcel(attendanceData, startDate, endDate) {
      ...
      <body>
        <table>
          <tr><td colspan="5" class="header">Attendance Record - ${systemTitle}</td></tr>
          <tr><td colspan="5" class="subheader">Date Range: ${startDate} to ${endDate}</td></tr>
          <tr>
      ...
      htmlContent += `
          <tr><td colspan="5" class="footer">Exported on ${new Date().toLocaleString()}</td></tr>
        </table>
      </body>
      </html>
      `;
    ...
    function exportToCSV(attendanceData, startDate, endDate) {
      let csvContent = "Date,Roll No,Student Name,Morning,Evening\n";
      
      attendanceData.forEach(record => {
        csvContent += `${record.date},${record.rollNo},${record.name},${record.morning},${record.evening}\n`;
      });
